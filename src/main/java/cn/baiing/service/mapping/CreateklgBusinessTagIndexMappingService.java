	...
	public static void createklgBusinessTagIndexMapping(){
		...
		try {
			XContentBuilder klgMapping = XContentFactory.jsonBuilder().startObject()
					.startObject(IndexRelationConstant.KLG_BUSINESSTAG_TYPR).startObject(IndexRelationConstant.PROPERTIES)
			 ...
			 PutMappingRequest mappingRequest = Requests.putMappingRequest(IndexRelationConstant.KLG_BUSINESSTAG_INDEX).type(IndexRelationConstant.KLG_BUSINESSTAG_TYPR).source(klgMapping);
